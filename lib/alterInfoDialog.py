...
class alterInfoDialog(QDialog):
    alter_Info_successful_signal = pyqtSignal()

    def __init__(self, parent=None):
        super(alterInfoDialog, self).__init__(parent)
        self.setUpUI()
    ...
    def setUpUI(self):
        ...
        # button控件
        self.alterInfoButton = QPushButton("确认")
        ...
        self.layout.addRow("", self.alterInfoButton)
        ...
        self.alterInfoButton.setFont(font)
        self.alterInfoButton.setFixedHeight(32)
        self.alterInfoButton.setFixedWidth(140)

        # 设置间距
        self.titleLable.setMargin(8)
        self.layout.setVerticalSpacing(10)

        self.alterInfoButton.clicked.connect(self.alterInfoButtonClicked)
    ...
    def alterInfoButtonClicked(self):
        ...
        if result == []:
            ...
            DB = SQLITE()
            DB.ExecNonQuery(f"UPDATE Users SET FirstName='{firstName}',LastName='{lastName}',Phone='{number}' WHERE ID = {ID}")
            del DB
            print(QMessageBox.warning(self, "提示", "修改成功!", QMessageBox.Yes, QMessageBox.Ok))
            self.clearEdit()
            return
